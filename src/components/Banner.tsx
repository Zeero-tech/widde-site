...
export default function Banner({ id, content, ctaLabel, ctaHref }: BannerProps) {
  ...
  return (
    <div className="sticky z-40 bg-brand text-white flex items-center justify-center gap-3 px-4 py-2.5 text-sm font-semibold min-h-[44px] relative" style={{ top: navHeight }}>
      ...
      {ctaLabel && (
        <a
          ...
          onClick={(e) => {
            e.preventDefault()
            window.dispatchEvent(new CustomEvent('open-vtex-map'))
          }}
