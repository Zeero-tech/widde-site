  ...
  return (
    <div className="bg-[#0A0A0A] rounded-[12px] md:rounded-[20px] px-6 py-6 md:px-16 md:py-14 flex flex-col md:flex-row items-center gap-5 md:gap-16">
      ...
      <div className="flex-1 md:order-1">
        <p className="text-xl md:text-2xl text-white leading-[1.4] font-normal mb-4 md:mb-6">
          {t("vc.quote.text")}
        </p>
        <div className="font-bold text-white mb-[2px]">
          {t("vc.quote.author")}
        </div>
        <div className="text-base md:text-base text-white/50 mb-4 md:mb-8">{t("vc.quote.company")}</div>
        <AnimatedButton
          href="https://widde.io/demo"
          className="inline-block w-full md:w-auto text-center bg-brand text-white font-bold text-lg md:text-base px-6 py-3 rounded-full no-underline hover:opacity-90 transition-opacity"
        >
          {t("vc.quote.cta")}
        </AnimatedButton>
      </div>
